...
//sourceImage stores the image passed into initWithImage: previewImageView displays the image with its current filter. photoFilterOperationQueue stores the photo filter operations; more on that later. filterCollectionView is a collection view that shows all of the filters available. filterImages and filterTitles hold filtered images and their titles. sendButton is the big purple "send to Instagram" button. sendBarButton shows on short iPhones in the navigation bar where there's no room for sendButton. documentController shares the image to Instagram
...
//This should look familiar, but let's walk through it. Store the source image passed in and initialize the preview image view with that image. Create the operation queue. Create and configure a UICollectionViewFlowLayout instance to define the layout of our filter collection view, and then use it to initialize a UICollectionView. Set the collection view's delegate and dataSource properties. (This would be unnecessary in a UICollectionViewController, but we're subclassing plain old UIViewController.) Disable showsHorizontalScrollIndicator to hide the horizontal scroll indicator; this is just an aesthetic preference. Initialize the filterImages and filterTitles arrays. The first object in each array represents the unfiltered image. (This will allow a user to remove a filter once it's applied.) Finally, create both sendButton and sendBarButton; they both have the same target-action method.
...
- (void)viewDidLoad {
    ...
    // #@#@#@#@#@#@#@#@#@# - assignment is to replace this with a sub class
...
//Between the image conversions and the filter itself, this code can take a few seconds to run. We don't want to run it on the main thread, and since we have a lot of filters, it could get annoying to keep track of everything using only dispatch_async. We'll use NSOperationQueue, which makes it very easy to schedule pieces of long-running code (or "operations", as they're called.) This code does the following: Converts the CIImage to a UIImage. Because CIImage isn't fully rendered, the output UIImage is slow to draw. Forces the UIImage to draw, and then saves the drawn UIImage. On the main thread, adds the completed UIImage and filter title to the arrays, and tells the collection view that a new item is available.
...
- (void) addFiltersToQueue {
    ...
    //We start off by creating a few filters and images. The CISepiaTone filter (#1) will make a sepia tone version of our source image. CIRandomGenerator (#2) makes a random image called randomImage. Its output looks like color TV static:At #3, we stretch the image a little bit horizontally, and a lot vertically. This second image is called otherRandomImage. At #4, CIColorMatrix creates two filters. whiteSpecks will extract white specks from randomImage, and darkScratches will extract vertical scratches from otherRandomImage. At #5, we create minimumComponent and composite which will combine the layers. After checking to ensure that those filters all exist (#6), start building. Apply the sepia filter (#7). At #8, we generate whiteSpecksImage (we crop it to the source image's size since the randomly generated image is infinite.) At #9, we create sepiaPlusWhiteSpecksImage by using CISourceOverCompositing to overlay the white specks on top of the sepia-toned image. At #10, we create darkScratchesImage and add it on top of the white specks, and call outputImage on the composite filter to get the whole thing.
